...
const FallbackMap = ({ latitude, longitude }) => {
  return (
...
const InvestmentOpportunitiesContent = () => {
  ...
  const openPropertyDetail = (property) => {
    ...
    fetchDocuments(property.id) // ← load docs here too
    onPropertyDetailOpen()
  }

  // Open documents modal
  const openDocumentsModal = async (property, e) => {
    e?.stopPropagation()
    try {
      const token = getToken()
  ...
  // Open owner details
  const openOwnerDetails = async (owner_id, e) => {
    e?.stopPropagation()
    try {
      const token = getToken()
  ...
  // Add to watchlist
  const addToWatchlist = async (propertyId, e) => {
    e?.stopPropagation()
    try {
      const token = getToken()
  ...
  useEffect(() => {
    ...
    // Sorting logic (now inside useEffect)
  ...
  const isInvestmentClosed = (opportunity) => {
    return (
  ...
  return (
    <Box bg={bgColor} minH="100vh" ref={topRef}>
              ...
              <VStack spacing={6} align="stretch" pt={4}>
                ...
                <HStack spacing={4}>
                  <Button
                    ...
                    onClick={() => {
                      onFilterDrawerClose()
                      // The useEffect will handle filtering based on the current state
                    }}
                  >
                    Apply Filters
                  </Button>
                ...
                >
                  ...
                  <Box position="relative" height="200px" overflow="hidden">
                    {opportunity.propertyImage ? (
                      <Image
                        src={`http://192.168.100.30:5000${opportunity.propertyImage}`}
                      ...
                      <Box bg="gray.700" display="flex" alignItems="center" justifyContent="center" height="100%">
      ...
      {/* Property Detail Modal - Completely redesigned for mobile */}
      <Modal
